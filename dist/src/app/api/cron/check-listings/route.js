...
export async function GET(request) {
    try {
        ...
        for (let i = 0; i < alerts.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.allSettled(batch.map(async (alert) => {
                try {
                    // Get listings for each alert
                    const listings = await scrapeListings(alert.cities, alert.category);
                    // Process and store the listings
                    await processListings(alert.id, listings, supabase);
