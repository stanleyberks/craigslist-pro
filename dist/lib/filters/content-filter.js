...
export class ContentFilter {
    ...
    containsBlacklistedPhrases(text) {
        const lowerText = text.toLowerCase();
        return BLACKLISTED_PHRASES.filter(phrase => lowerText.includes(phrase.toLowerCase()));
    }
    async validateImageUrls(listing) {
        if (!listing.pics || listing.pics.length === 0) {
            return { hasValidImages: false, invalidImageUrls: [] };
        }
        const invalidImageUrls = [];
        const validImageUrls = [];
        await Promise.all(listing.pics.map(async (url) => {
            try {
...
Return only a number between 0 and 1.`;
    ...
    isDuplicate(listing, windowHours) {
        const now = Date.now();
        const windowMs = windowHours * 60 * 60 * 1000;
        // Clean old entries
        for (const [key, timestamps] of this.dedupeCache.entries()) {
            const validTimestamps = new Set([...timestamps].filter(ts => now - parseInt(ts) < windowMs));
    ...
    async filter(listing, options = {}) {
        const result = {
            isFiltered: false,
            reasons: []
        };
        // Check blacklisted phrases
        const blacklistedPhrases = this.containsBlacklistedPhrases([listing.title, listing.description].join(' '));
        if (blacklistedPhrases.length > 0) {
            result.isFiltered = true;
            result.reasons.push(`Contains blacklisted phrases: ${blacklistedPhrases.join(', ')}`);
        }
        // Check for duplicates
    ...
    async filterListings(listings, options = {}) {
        ...
        for (const batch of batches) {
            const results = await Promise.all(batch.map(listing => this.filter(listing, options)));
