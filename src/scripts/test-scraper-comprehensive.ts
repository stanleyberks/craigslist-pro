...
import type { CraigslistListing } from '../lib/types.js';
import type { CategoryKey } from '../lib/categories.js';
...
async function runTest(city: string, category: CategoryKey): Promise<TestResult> {
  const MAX_RETRIES = 3;
  ...
  try {
    // Test both scrapers
    let listings: CraigslistListing[] = [];
    let imageListings: CraigslistListing[] = [];
    let retries = 0;
...
function getMockListings(): CraigslistListing[] {
  return [
    {
      id: '1',
