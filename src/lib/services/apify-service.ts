import { CraigslistListing } from '../types.js';
...
export class ApifyService {
  ...
  private readonly actorId = APIFY_ACTOR_ID;
   ...
   */
  async scrapeListings(input: ApifyInput, options: ScrapeOptions = {}): Promise<CraigslistListing[]> {
    try {
      // Start the actor run
   ...
   */
  private async startActorRun(input: ApifyInput) {
   ...
   */
  private async waitForRunCompletion(runId: string, maxAttempts = 30): Promise<{ id: string }> {
   ...
   */
  private async getDatasetItems(datasetId: string, limit?: number): Promise<any[]> {
   ...
   */
  private transformListings(items: any[]): CraigslistListing[] {
